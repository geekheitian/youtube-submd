...
from workers.tasks import dispatch_transcript_job
...
async def create_transcript(
    ...
    for session in get_session():
        existing = find_active_job(session, "transcript", video_id)
        if existing:
            return JobResponse(
        ...
        job = create_job(session, "transcript", video_id, source_url, api_key)
        dispatch_transcript_job(str(job.id), video_id, source_url)
        return JobResponse(
            job_id=str(job.id),
